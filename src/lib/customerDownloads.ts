...
import {
  DocumentData, DocumentLineItem, DocumentType, downloadDocumentPdf, setPdfSettings,
} from '@/lib/documentPdf';
import { downloadQuotationPdf } from '@/lib/quotationPdf';
import { certificateToDocData, CertificateRecord } from '@/lib/serviceCertificates';
...
function toLineItems(raw: any, fallbackName: string, amount: number): DocumentLineItem[] {
  ...
  return [{ name: fallbackName, quantity: 1, unitPrice: amount, total: amount }];
}
...
export function downloadCustomerInvoice(inv: any) {
  ...
  const data: DocumentData = {
    ...
    createdBy: inv.salesperson_name || 'Concept Cleaning Services',
    createdByRole: inv.salesperson_role || 'staff',
    clientName: inv.client_name,
    clientPhone: inv.client_phone,
    lineItems: toLineItems(inv.line_items, inv.service || 'Cleaning Service', amount),
    totalAmount: amount,
...
export function downloadCustomerDocument(doc: any) {
  ...
  const type = String(doc.document_type || 'receipt') as DocumentType;
  const data: DocumentData = {
    documentType: type,
    documentNumber: doc.document_number,
    dateCreated: safeDate(doc.created_at),
    createdBy: doc.created_by_name || 'Concept Cleaning Services',
    ...
    lineItems: toLineItems(doc.line_items, doc.service_name || doc.description || 'Service', amount),
    totalAmount: amount,
...
export function downloadCustomerQuotation(q: any) {
  downloadQuotationPdf({
    ...
    serviceDate: q.service_date ? safeDate(q.service_date) : '',
    price: Number(q.price) || 0,
    subtotal: q.subtotal != null ? Number(q.subtotal) : undefined,
    discountAmount: Number(q.discount_amount) || 0,
    discountReason: q.discount_reason || undefined,
    lineItems: Array.isArray(q.line_items) && q.line_items.length
      ? q.line_items.map((i: any) => ({
          name: i.service_name || i.name || q.service_name,
          quantity: i.quantity ?? 1,
          unitPrice: Number(i.unit_price ?? i.unitPrice ?? i.total) || 0,
...
export function downloadCustomerCertificate(cert: any) {
  const data = certificateToDocData(cert as CertificateRecord);
