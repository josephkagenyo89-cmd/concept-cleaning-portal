...
function drawFooter(doc: jsPDF, w: number, h: number, pageNum: number, totalPages: number, data: DocumentData) {
  const brand = hexToRgb(pdfSettings.document.primary_color || '#2A9D8F');
  const g = pdfSettings.general;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...DARK);
  doc.text(`Verification Code: ${verificationCode}`, 18, footerY + 5);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(...GRAY_TEXT);
  doc.text(`Generated: ${generatedAt} • By: ${data.createdBy}`, 18, footerY + 9);

  const contactBits = [g.phone, g.email, g.website].filter(Boolean).join('  •  ');
  const maxFooterWidth = qrX - 24;
  if (contactBits) doc.text(contactBits, 18, footerY + 13, { maxWidth: maxFooterWidth });
  doc.setFont('helvetica', 'italic');
  doc.text('Generated by Concept Cleaning Services Management System', 18, footerY + 17, { maxWidth: maxFooterWidth });
  if (docFooter) {
    doc.setFont('helvetica', 'normal');
    doc.text(docFooter, 18, footerY + 21, { maxWidth: maxFooterWidth });
  }
  doc.text(`Page ${pageNum} / ${totalPages}`, w / 2, h - 6, { align: 'center' });
}
